...
fn handle_blockchain_request(
    ...
    match req {
        ...
        BlockchainWriteRequest::BatchWriteBlock(blocks) => batch_write_block(env, blocks),
...
/// [`BlockchainWriteRequest::BatchWriteBlock`].
#[inline]
fn batch_write_block(env: &ConcreteEnv, blocks: &[VerifiedBlockInformation]) -> ResponseResult {
    let env_inner = env.env_inner();
    let tx_rw = env_inner.tx_rw()?;
