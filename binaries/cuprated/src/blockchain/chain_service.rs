...
use crate::blockchain::fast_sync::validate_entries;
    ...
    fn call(&mut self, req: ChainSvcRequest<N>) -> Self::Future {
        ...
        match req {
            ...
            ChainSvcRequest::ValidateEntries(entries, start_height) => {
                ...
                async move {
                let (valid, unknown)  = validate_entries(entries, start_height, &mut blockchain_read_handle).await?;

                Ok(ChainSvcResponse::ValidateEntries {
                    valid,
                    unknown
                })
            }.boxed()
