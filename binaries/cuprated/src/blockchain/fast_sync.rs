...
use cuprate_fast_sync::fast_sync::{FastSyncError, FastSyncResponse};
...
pub async fn validate_entries<N: NetworkZone>(
    ...
    let hashes_stop_height = min( (last_height / FAST_SYNC_BATCH_LEN) * FAST_SYNC_BATCH_LEN, FAST_SYNC_TOP_HEIGHT);
...
pub fn block_to_verified_block_information(block: Block, mut txs: Vec<Transaction>, blockchin_ctx: &BlockchainContext) -> VerifiedBlockInformation {
    ...
    let mut txs = txs.into_iter().map(|tx| {
    ...
    for tx in &block.transactions {
        let data = txs
            .remove(tx)
            .expect("fast sync block invalid");
    ...
    VerifiedBlockInformation {
        ...
        cumulative_difficulty: blockchin_ctx.cumulative_difficulty
            + blockchin_ctx.next_difficulty,
        block,
    }
}
